...
public class VisualizerFrame extends JFrame {
    ...
    public void preDrawArray(Integer[] squares) {
        ...
        for (int i = 0; i < SortingVisualizer.sortDataCount; i++) {
            ...
            squarePanels[i].setBackground(new Color(135, 206, 235));
    ...
    public void reDrawArray(Integer[] squares, int working, int comparing, int reading) {
        ...
        for (int i = 0; i < squarePanels.length; i++) {
            ...
            squarePanels[i].setPreferredSize(new Dimension(SortingVisualizer.blockWidth, squares[i] * sizeModifier));
            if (i == working) {
                squarePanels[i].setBackground(Color.cyan); // Working element in Cyan
                squarePanels[i].setBorder(new LineBorder(new Color(57, 255, 20), 2)); // Neon Green border
            } else if (i == comparing) {
                squarePanels[i].setBackground(Color.magenta); // Comparing element in Magenta
                squarePanels[i].setBorder(new LineBorder(new Color(57, 255, 20), 2)); // Neon Green border
            } else if (i == reading) {
                squarePanels[i].setBackground(Color.orange); // Reading element in Orange
                squarePanels[i].setBorder(new LineBorder(new Color(57, 255, 20), 2)); // Neon Green border
            } else {
                squarePanels[i].setBackground(new Color(135, 206, 235)); // Default Sky Blue color
                squarePanels[i].setBorder(new LineBorder(Color.darkGray, 1)); // Dark Gray border
            }
            arrayWrapper.add(squarePanels[i], c);
        }
        repaint();
